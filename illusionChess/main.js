...
//  document.getElementById("t").innerHTML = "JS Works!"; //Silly Penguins Debugging
//});
    ...
    for (var rowDataIndex = 0; rowDataIndex < rows.length; rowDataIndex++) {
      ...
      for (var cellDataIndex = 0; cellDataIndex < rowPositions.length; cellDataIndex++) {
        ...
        if (validPieceIDs.includes(rowPositions[cellDataIndex].toLowerCase())) {
          const char = rowPositions[cellDataIndex];
          // Determine color: lowercase = black, uppercase = white
          const pieceColour = (char === char.toLowerCase() ? 'b' : 'w');
          // Force the letter part to uppercase
          const pieceLetter = char.toUpperCase();
          // Store e.g. "bP", "wN", etc.
          boardData[boardRowIndex][boardCellIndex] = pieceColour + pieceLetter;
          boardCellIndex++;
        } else {
